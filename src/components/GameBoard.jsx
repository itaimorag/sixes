...
import { applyStopBonusesAndPenalties, } from "@/lib/game-logic";
import { Card, CardContent, CardDescription, CardHeader, CardTitle, } from "@/components/ui/card";
import { RotateCcw, PartyPopper, Shuffle, HelpCircle, } from "lucide-react";
import { PlayerHand } from "./CardDisplay";
import { createDeck, shuffleDeck } from "@/lib/card-utils";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {players.map((player) => (<div key={player.id} className="p-4 border rounded-lg bg-card/50">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                return (<Image key={card.id} src={cardImageSrc} alt={cardAltText} width={70} // Updated card width
                 height={100} // Updated card height
                 className="rounded shadow-md" data-ai-hint={dataAiHint}/>);
            })}
                {(!player.hand || player.hand.length === 0) &&
                Array.from({ length: 6 }).map((_, i) => (<Image key={`empty-${player.id}-${i}`} src="https://placehold.co/70x100.png" alt="Face-down card placeholder" width={70} height={100} className="rounded shadow-md opacity-50" data-ai-hint="card back"/>))}
              </div>
            </div>))}
        </div>
...
export function GameBoard({ initialPlayers, onNewGame, currentPlayerId, }) {
    ...
    const handleScoresSubmit = (roundScores) => {
        ...
        if (gameState.gameStatus === "final_round") {
            const finalPlayersWithBonuses = applyStopBonusesAndPenalties(updatedPlayers, ((_a = updatedPlayers.find((p) => p.isStopper)) === null || _a === void 0 ? void 0 : _a.id) || "");
    ...
    if (gameState.gameStatus === "game_over") {
        ...
        <Scoreboard players={gameState.players} currentRound={gameState.roundNumber - 1}/>
    ...
    return (<div className="space-y-8">
      ...
      <div className="flex justify-center items-center gap-12 mb-8">
        ...
        <div className="flex flex-col items-center">
          <Image src={CARD_BACK_IMAGE} alt="Deck (face down)" width={80} height={100} className="rounded shadow-md"/>
        ...
        <div className="flex flex-col items-center">
          {gameState.discardPile.length > 0 ? (<Image src={gameState.discardPile[gameState.discardPile.length - 1].imageSrc} alt="Top of Discard Pile" width={80} height={100} className="rounded shadow-md"/>) : (<div className="w-[80px] h-[100px] rounded shadow-md bg-muted flex items-center justify-center text-muted-foreground">
              Empty
            </div>)}
          <span className="mt-2 text-sm text-muted-foreground">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {gameState.players.map((player) => (<Card key={player.id} className="p-4">
            <CardHeader>
            ...
            <CardContent>
              <PlayerHand cards={player.hand} isCurrentPlayer={player.id === currentPlayerId} hasViewedBottomRow={!player.hasViewedBottomRow} hasSwappedTopRow={player.hasSwappedTopRow} onCardClick={player.id === currentPlayerId ? handleCardClick : undefined}/>
            </CardContent>
          </Card>))}
      </div>

      {currentPlayer && !currentPlayer.hasViewedBottomRow && (<Card className="bg-accent/10">
      ...
      {currentPlayer && currentPlayer.hasViewedBottomRow && (<Card>
          <CardHeader>
            <CardTitle>Your Turn</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex gap-4">
              <Button onClick={handleDrawFromDeck} disabled={!!gameState.deck.length}>
                Draw from Deck ({gameState.deck.length})
              </Button>
              <Button onClick={handleDrawFromDiscard} disabled={!!gameState.deck.length}>
            ...
            {gameState.deck.length > 0 && (<div className="space-y-4">
                <div className="flex justify-center">
                  <PlayerHand cards={gameState.deck.slice(0, 3)} isCurrentPlayer={true} hasViewedBottomRow={true} hasSwappedTopRow={true}/>
                </div>
            ...
            <div className="flex justify-center">
              <Button onClick={handleCallStop} variant="destructive" disabled={gameState.gameStatus !== "playing"}>
                Call STOP
              </Button>
            </div>
          </CardContent>
      ...
      <Scoreboard players={gameState.players} currentRound={gameState.roundNumber}/>

      {(gameState.gameStatus === "playing" ||
            gameState.gameStatus === "final_round") && (<RoundScoreForm players={gameState.players} currentRound={gameState.roundNumber} onSubmitScores={handleScoresSubmit} isFinalRound={gameState.gameStatus === "final_round"}/>)}
    </div>);
}
