...
export function Scoreboard({ players, currentRound }) {
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {Array.from({ length: maxRounds }, (_, i) => (<TableHead key={`round-header-${i}`} className="text-center font-headline">R{i + 1}</TableHead>))}
                <TableHead className="text-right font-headline">Total</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {sortedPlayers.map((player, index) => (<TableRow key={player.id} className={player.isStopper ? 'bg-accent/20' : ''}>
                  ...
                  {Array.from({ length: maxRounds }, (_, i) => (<TableCell key={`score-${player.id}-${i}`} className="text-center">
