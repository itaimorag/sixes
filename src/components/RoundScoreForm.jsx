...
export function RoundScoreForm({ players, currentRound, onSubmitScores, isFinalRound }) {
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          {players.map(player => (<div key={player.id} className="space-y-2">
              ...
              <Input id={`score-${player.id}`} type="number" value={roundScores[player.id] || ''} onChange={(e) => handleScoreChange(player.id, e.target.value)} placeholder="Enter score" required className="text-base h-12"/>
            </div>))}
