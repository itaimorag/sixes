...
export function GamePageClient() {
    ...
    return (<div className="container mx-auto p-4 md:p-8 flex flex-col items-center">
      ...
      <main className="w-full max-w-3xl">
        {viewState === "setup" && (<PlayerSetup onSetupComplete={handleSetupComplete}/>)}
        {viewState === "player_selection" && players.length > 0 && (<PlayerIdentitySelection players={players} onPlayerSelected={handlePlayerSelected}/>)}
        {(viewState === "playing" ||
            viewState === "final_round" ||
            viewState === "game_over") &&
            players.length > 0 &&
            currentPlayerId && (<GameBoard key={gameId} initialPlayers={players} onNewGame={handleNewGame} currentPlayerId={currentPlayerId}/>)}
      </main>
