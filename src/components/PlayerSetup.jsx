...
const MAX_PLAYERS = 4;
const MIN_PLAYERS = 3;
export function PlayerSetup({ onSetupComplete }) {
    ...
    return (<Card className="w-full max-w-md shadow-xl">
      <CardHeader>
        <CardTitle className="text-3xl flex items-center">
          <Users className="mr-2 h-8 w-8 text-primary"/> Player Setup
        </CardTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <Select onValueChange={handleNumPlayersChange} defaultValue={String(MIN_PLAYERS)}>
              ...
              <SelectContent>
                {Array.from({ length: MAX_PLAYERS - MIN_PLAYERS + 1 }, (_, i) => MIN_PLAYERS + i).map((num) => (<SelectItem key={num} value={String(num)}>
                    {num} Players
                  </SelectItem>))}
              </SelectContent>
            </Select>
          ...
          {playerNames.map((name, index) => (<div key={index} className="space-y-2">
              ...
              <Input id={`player-${index}`} type="text" value={name} onChange={(e) => handleNameChange(index, e.target.value)} placeholder={`Enter Player ${index + 1}'s Name`} required className="text-base"/>
            </div>))}
